        ...
        <?php foreach($events as $event): ?>
            ...
            ?>
                <?php foreach($data_start as $key => $start): ?>
                {
                    ...
                    end: '<?php $date_end_value = new DateTime($data_end[$key]); if($event['event_type'] == 'Continuous' ){ $date_end_value->modify('+1 day'); } echo $date_end_value->format('Y-m-d') . 'T' . $data_time[$key];; ?>'
                },
                <?php endforeach; ?>
            <?php } ?>
        <?php endforeach; ?>
