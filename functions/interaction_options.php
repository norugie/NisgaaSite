    ...
    if(isset($_GET['interaction'])){
        ...
        if(isset($_GET['editCarouselImage'])){
            ...
            if(!file_exists($_FILES['carousel_image']['tmp_name']) || !is_uploaded_file($_FILES['carousel_image']['tmp_name'])){
                ...
                if(isset($_POST['cropped_image_value_edit'])){
                    $data = $_POST['cropped_image_value_edit'];
                    ...
                    $imageName = "CRSL-EDIT-" . rand(111111,99999) . '.png';
        ...
        if(isset($_GET['policyDisable'])){
            ...
            $interaction->disablePolicy($database, $id, $title);
        ...
        if(isset($_GET['policyReactivate'])){
            ...
            $interaction->reactivatePolicy($database, $id, $title);
        ...
        if(isset($_GET['editPolicy'])){
            ...
            $interaction->editPolicy($database, $id, $link_id, $link_name, $link_desc, $link_content, $link_tag);
            ...
            if(file_exists($_FILES['link_content']['tmp_name']) || is_uploaded_file($_FILES['link_content']['tmp_name'])){
                if(isset($_FILES['link_content'])){
                    ...
                    if($errors == 0){
                        move_uploaded_file($file_tmp, "../links/".$new_file_name);
                        $link_content = mysqli_real_escape_string($database->con, $new_file_name);
                        $interaction->addPolicy($database, $link_name, $link_desc, $link_content, $link_type, $link_tag, $link_thumbnail);
                    } else {
                        if($error == 1){
                            $_SESSION['error_message'] = "You tried uploading a file with an invalid file extension. Please make sure that the file's extension is one of the followings: .doc, .docx, .pdf.";
                        } else if($error == 2){
