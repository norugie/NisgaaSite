<?php
    ...
    if(isset($_GET['post'])){
        ...
        if(isset($_GET['addPostIntegrated'])){
            ...
            $nlc_opt;
            if(isset($_POST['post_nlc_autopost']) && !empty($_POST['post_nlc_autopost']) ? $nlc_opt = mysqli_real_escape_string($database->con, $_POST['post_nlc_autopost']) : $gcc_opt = "No" );
            $post_thumbnail;
            $post_desc;
            ...
            if(!file_exists($_FILES['post_thumbnail']['tmp_name']) || !is_uploaded_file($_FILES['post_thumbnail']['tmp_name'])){
                ...
                $post_id = $post->addPostIntegrated($database, $post_title, $post_content, $post_thumbnail, $post_desc, $post_type, $sm_opt, $ssd_opt, $ss_opt, $gcc_opt, $nlc_opt);
            } else {
